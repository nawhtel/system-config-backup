...
SRC_URI="amd64? ( ${BASE_URI}/boot.amd64-unix.tgz -> ${P}-boot.amd64-unix.tgz )
...
src_compile() {
	SMLNJ_HOME="${S}" \
		./config/install.sh -default 64 || die "compilation failed"
}
...
src_install() {
	...
	exeinto ${DIR}/bin
	pushd bin || die
	for i in .arch-n-opsys .link-sml .run-sml asdlgen ml-* sml; do
		doexe ${i}
	done
	for i in asdlgen ml-* sml; do
